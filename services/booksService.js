...
function getGoogleBooks(bookNameSearch) {
    if(bookNameSearch === "") bookNameSearch='java script'
    var prmBooks = axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookNameSearch}`)
    return  prmBooks.then((response)=>{
...
function getBookById(bookId) {
    if(storageService.load('books')) gBooks = storageService.load('books');
    const book = gBooks.find(book => book.id === bookId);
    return Promise.resolve(book);
}

function getBooks(filterBy) {
    if(storageService.load('books')) {
        gBooks = storageService.load('books');
    } else {
        storageService.store('books',gBooks);
    }
    if (!filterBy) return Promise.resolve([...gBooks]);
    const filteredBooks = gBooks.filter(book=> book.title.includes(filterBy.name) 
